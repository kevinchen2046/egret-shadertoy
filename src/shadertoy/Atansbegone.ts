module shadertoy {
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord ,in sampler2D iChannel0) {
      vec2 f=vec2(fragCoord.x,fragCoord.y*(uTextureSize.y/uTextureSize.x));
      vec2 uv = f/RESOLUTION.xy;
      vec2 p = -1. + 2. * vec2(uv.x+0.5,uv.y-0.5);
      vec2 pp = p;
      p.x *= RESOLUTION.x/RESOLUTION.y;
    
      vec3 col = effect(p, pp);
      vec4 tColor=texture2D(iChannel0,fragCoord);
      fragColor = vec4(col, 1.0)+tColor;
    }`
}