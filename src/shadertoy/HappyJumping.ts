...
#define AA 1.0
// #else
// #define AA 2.0  // Set AA to 1 if your machine is too slow
// #endif
...
vec4 raycast( in vec3 ro, in vec3 rd, float time )
{
    ...
    // raymarch scene
    float t = tmin;
    for( float i=0.0; i<TOTAL2 && t<tmax; i++ )
    {
...
// #else
    ...
    vec3 n = vec3(0.0);
    int ZERO1 = int(ZERO);
    for(int i=ZERO1; i<4; i++)
    {
        // float x=((float(i)+3.0)*0.5)&1.0;
        // float y=(i*0.5)&1.0;
        // float z=i&1.0;
        float x=((float(i)+3.0)*0.5);
        float y=(float(i)*0.5);
        float z=float(i);
        vec3 e = 0.5773*(2.0*vec3(x,y,z)-1.0);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    vec3 tot = vec3(0.0);
// #if AA>1
//     for( float m=ZERO; m<AA; m++ )
//     for( float n=ZERO; n<AA; n++ )
//     {
//         // pixel coordinates
